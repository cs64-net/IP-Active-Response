"""Juniper SRX firewall client for managing IP blocks via SSH.

Uses Paramiko to connect via SSH and manages address-set entries under
``security address-book global`` (address_group mode) or static routes
to ``discard`` (null_route mode) via the Junos CLI.
...
BATCH_SIZE = 200
MAX_ADDRESS_SET_ENTRIES = 7500
COMMAND_DELAY = 0.5
READ_TIMEOUT = 10
COMMIT_TIMEOUT = 60


class JuniperSrxError(Exception):
    """Custom exception for Juniper SRX client operations."""
    pass


class JuniperSrxClient(BaseDeviceClient):
    """Client for managing IP blocks on Juniper SRX firewalls via SSH.

    Supports two block methods:

    - ``address_group``: Creates address entries under
      ``security address-book global`` with ``SOC_`` prefix naming and
      adds them to an address-set. The administrator must reference the
      address-set in a Junos security policy.
    - ``null_route``: Creates static routes pointing blocked IPs to the
      ``discard`` next-hop.

    Address objects are named with a ``SOC_`` prefix where dots are
    replaced by underscores (e.g. ``SOC_192_168_1_1`` for IP
    ``192.168.1.1``) so they can be identified and cleaned up.

    When the number of entries in a single address-set exceeds
    ``MAX_ADDRESS_SET_ENTRIES`` (7,500), entries are split across
    multiple address-sets with numeric suffixes.
    """
    ...
    def __init__(self, host: str, port: int, username: str,
        ...
        """Initialize SSH connection parameters for Juniper SRX.

        Args:
            host: Hostname or IP of the SRX firewall.
            port: SSH port number.
            username: SSH username.
            password: SSH password.
            address_group_name: Name of the address-set or route tag
                (default: SOC_BLOCKLIST).
            block_method: ``'address_group'`` to manage address-set
    ...
    def _address_object_name(ip: str) -> str:
        """Return the Junos address object name for an IP.

        Dots and slashes are replaced with underscores so the name is
        safe for Junos configuration.  CIDR /32 suffixes are stripped.

        Examples:
            '192.168.1.1'     -> 'SOC_192_168_1_1'
            '10.0.3.0/24'     -> 'SOC_10_0_3_0_24'
        ...
        if "/" in ip:
            parts = ip.split("/")
            if parts[1] == "32":
                return "SOC_" + parts[0].replace(".", "_")
            return "SOC_" + parts[0].replace(".", "_") + "_" + parts[1]
        return "SOC_" + ip.replace(".", "_")
    ...
    def _get_ssh_shell(self) -> tuple:
        ...
        Raises:
            JuniperSrxError: If the SSH connection fails.
        """
    ...
    def _commit(self, shell) -> str:
        """Issue a ``commit`` command on the Juniper SRX.
        ...
        Raises:
            JuniperSrxError: If the commit fails.
        """
    ...
    def _address_set_name(self, index: int) -> str:
        """Return the address-set name for a given split index.
        ...
        Returns:
            Address-set name, e.g. ``SOC_BLOCKLIST`` or
            ``SOC_BLOCKLIST_2``.
        """
        if index == 0:
    ...
    def add_rules_bulk(self, ip_addresses: list) -> dict:
        """Create address entries / static routes and commit.

        In ``address_group`` mode, creates address objects under
        ``security address-book global`` with ``SOC_`` prefix naming
        and adds each to the address-set.  When entries exceed
        ``MAX_ADDRESS_SET_ENTRIES``, splits across multiple
        address-sets with numeric suffixes.
    ...
    def _add_batch_address_group(self, shell, batch: list,
        ...
        Creates address objects and adds them to the address-set,
        splitting across multiple address-sets when the limit is
        exceeded.
        """
        # Track which address-set index we're currently filling.
        # We use a simple counter based on total successful adds so far.
        ...
        for ip in batch:
            try:
                ...
                # Determine which address-set to use (group splitting)
                set_index = current_total // MAX_ADDRESS_SET_ENTRIES
                set_name = self._address_set_name(set_index)

                # Add to address-set
                cmd = (
                    f"set security address-book global address-set"
                    f" {set_name} address {obj_name}"
                )
                output = self._send_command(shell, cmd)
                if "error" in output.lower() or "invalid" in output.lower():
                    results["failed"].append(
    ...
    def remove_rules_bulk(self, ip_addresses: list) -> dict:
        """Remove address entries / static routes and commit.

        In ``address_group`` mode, removes address entries from the
        address-set and deletes the address objects.
    ...
    def _remove_batch_address_group(self, shell, batch: list,
        ...
        Removes each address from all possible address-sets (base name
        and numbered suffixes) and deletes the address object.  Missing
        entries are treated as skipped.
        """
        for ip in batch:
            try:
                obj_name = self._address_object_name(ip)

                # Try to remove from the base address-set and numbered
                # variants.  Junos silently accepts deletes of
                # non-existent members, so we attempt the base name and
                # a few numbered suffixes.
                for idx in range(10):
                    set_name = self._address_set_name(idx)
                    cmd = (
                        f"delete security address-book global"
                        f" address-set {set_name} address {obj_name}"
                    )
                    output = self._send_command(shell, cmd)
                    # "warning: statement not found" is expected for
                    # sets that don't contain this address — not an error.

                # Delete the address object itself
                cmd = (
                    f"delete security address-book global address"
                    f" {obj_name}"
                )
                output = self._send_command(shell, cmd)
                lower = output.lower()
    ...
    def check_health(self) -> bool:
        """Verify SSH connectivity to the Juniper SRX.
    ...
    def cleanup(self) -> bool:
        ...
        In ``address_group`` mode, deletes all ``SOC_`` prefixed
        addresses and address-sets from ``security address-book global``.

        In ``null_route`` mode, deletes all ``SOC_`` managed static
        routes.
    ...
    def _cleanup_address_group(self, shell) -> None:
        """Delete all SOC_ address-sets and address objects."""
        # First, list address-sets and delete SOC_ ones
        output = self._send_command(
            shell,
            "show security address-book global address-set",
        )
        for line in output.splitlines():
            stripped = line.strip()
            # Look for address-set names that match our pattern
            if self.address_group_name in stripped:
                # Extract the set name — lines like:
                #   address-set SOC_BLOCKLIST { ... }
                tokens = stripped.split()
                for token in tokens:
                    if token.startswith(self.address_group_name):
                        set_name = token.rstrip("{;}")
                        self._send_command(
                            shell,
                            f"delete security address-book global"
                            f" address-set {set_name}",
                        )
                        break

        # Then delete all SOC_ address objects
        output = self._send_command(
            shell,
            "show security address-book global address",
        )
        for line in output.splitlines():
            stripped = line.strip()
            if "SOC_" in stripped:
    ...
    def _cleanup_null_route(self, shell) -> None:
        ...
        for line in output.splitlines():
            ...
            if "discard" in stripped.lower():
                # Extract the route prefix — lines like:
                #   route 192.168.1.1/32 discard;
                tokens = stripped.split()
