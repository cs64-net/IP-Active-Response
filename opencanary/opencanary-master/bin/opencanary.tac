...
# ── Periodic heartbeat ──
# Re-send the boot event every 300 seconds so the SOC dashboard knows
# this instance is still alive.  Uses reactor.callWhenRunning to ensure
# the LoopingCall only starts after the Twisted reactor is fully up.
HEARTBEAT_INTERVAL = 300  # seconds

def _heartbeat():
    try:
        data = {"logtype": 1000, "src_host": "127.0.0.1", "dst_host": "127.0.0.1"}
        data["logdata"] = {"msg": "Canary heartbeat"}
        logger.log(data, retry=False)
    except Exception:
        pass  # Don't let heartbeat errors crash the service

def _start_heartbeat():
    try:
        loop = task.LoopingCall(_heartbeat)
        loop.start(HEARTBEAT_INTERVAL, now=False)
    except Exception:
        pass

from twisted.internet import reactor
reactor.callWhenRunning(_start_heartbeat)
